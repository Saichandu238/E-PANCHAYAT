...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $checkQuery = "SELECT * FROM sro_details WHERE Employee_Id = ?";
     ...
     if ($result->num_rows > 0) {
         ...
         echo "SRO already registered with this Authentication ID.";
     } else {


     // Check if the Aadhaar number already exists
     $checkQuery = "SELECT * FROM sro_details WHERE Aadhar_Number = ?";
     $stmt = $conn->prepare($checkQuery);
     ...
     if ($result->num_rows > 0) {
         // Aadhaar number already exists
         echo "SRO already registered with this Aadhar number.";
     } else {

         // Check if the Mail ID already exists
     $checkQuery = "SELECT * FROM Sro_details WHERE Mail_id = ?";
     $stmt = $conn->prepare($checkQuery);
     ...
     if ($result->num_rows > 0) {
         // Aadhaar number already exists
         echo "SRO already registered with this Mail ID";
     } else {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO sro_details (Employee_Id, Name, Aadhar_Number, DOB, Joining_date, Village, Mandal, District,  Mail_id, Password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        echo "SRO registered successfully.";
    } else {
