...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "DELETE FROM statemanager WHERE SManager_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $SManager_id);

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $deletion_message = "State Manager details with Employee ID $SManager_id have been successfully deleted.";
        } else {
            $deletion_message = "No State Manager found with Employee ID $SManager_id.";
        }
    } else {
        $deletion_message = "Error deleting State Manager details: " . $conn->error;
    }
...
<head>
    ...
    <title>Delete State Manager Details</title>
    <style>
        body {
...
<div class="delete-sro-box">
    <h3>Delete State Manager Details</h3>
    <form method="POST" action="">
        <label for="SManager_id">Enter Staate Manager ID:</label>
        <input type="text" id="SManager_id" name="SManager_id" required>
