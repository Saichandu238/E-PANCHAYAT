...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "DELETE FROM user_registration WHERE user_id = ? and Mandal= '$Mandal'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $employee_id);

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $deletion_message = "user details with user ID $employee_id have been successfully deleted.";
        } else {
            $deletion_message = "No user  found with Employee ID $employee_id.";
        }
    } else {
