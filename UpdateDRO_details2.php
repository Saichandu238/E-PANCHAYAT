...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['Employee_Id'])) {
    ...
    $sql = "UPDATE dro_details SET  Name = ?, Aadhar_Number = ?, DOB= ?, Joining_date=?, Village=?, Mandal=?, District=?, Gmail=?, Password = ? WHERE Employee_Id = ? ";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssss", $name, $Aadhar_Number, $DOB, $Joining_date, $Village, $Mandal, $District, $Mail_id, $Password, $Employee_Id);


    if ($stmt->execute()) {
        echo "<p>Details updated successfully. <a href='DRO_Profile.php'> Click here to go back</p>";
