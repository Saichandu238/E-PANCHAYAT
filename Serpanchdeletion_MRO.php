...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $employee_id = $_POST['Serpanch_id'];

    // Prepare and execute the delete query
    $sql = "DELETE FROM serpanchdetails WHERE Id = ? and Mandal= '$Mandal'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $employee_id);

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $deletion_message = "Serpanch details with Employee ID $employee_id have been successfully deleted.";
        } else {
            $deletion_message = "No Serpanch found with Employee ID $employee_id.";
        }
    } else {
...
<head>
    ...
    <title>Delete Serpanch Details</title>
    <style>
        body {
...
<div class="delete-sro-box">
    <h3>Delete Serpanch Details</h3>
    <form method="POST" action="">
        <label for="Serpanch_id">Enter Serpanch ID:</label>
        <input type="text" id="Serpanch_id" name="Serpanch_id" required>
        <input type="submit" value="Delete Serpanch">
    </form>
