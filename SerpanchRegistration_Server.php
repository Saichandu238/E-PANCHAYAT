...
function generateUniqueID($conn) {
    do {
        ...
        $query = "SELECT * FROM Serpanchdetails WHERE ID = '$unique_id'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $password = $_POST['Password'];
     ...
     $checkQuery = "SELECT * FROM Serpanchdetails WHERE Aadhar_number = ?";
     ...
     if ($result->num_rows > 0) {
         ...
         echo "Serpanch already registered with this Aadhaar number.";
     } else {

         // Check if the Mail ID already exists
     $checkQuery = "SELECT * FROM Serpanchdetails WHERE Gmail = ?";
     $stmt = $conn->prepare($checkQuery);
     ...
     if ($result->num_rows > 0) {
         // Aadhaar number already exists
         echo "Serpanch already registered with this Mail ID";
     } else {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO Serpanchdetails (ID, Name, Aadhar_number, Joining_date, ending_date, Village, Mandal, District, Number, Gmail, Password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        echo "Serpanch registered successfully. Your Serpanch ID is: " . $admin_id ." Please Save the ID for further Reference";
    } else {
