...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['Employee_Id'])) {
    ...
    $sql = "UPDATE sro_details SET  Name = ?, Aadhar_Number = ?, DOB= ?, Joining_date=?, Village=?, Mandal=?, District=?, Mail_id=?, Password = ? WHERE Employee_Id = ? ";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssss", $name, $Aadhar_Number, $DOB, $Joining_date, $Village, $Mandal, $District, $Mail_id, $Password, $Employee_Id);


    if ($stmt->execute()) {
        echo "<p>Details updated successfully. <a href='SRO_Profile.php'> Click here to go back</p>";
