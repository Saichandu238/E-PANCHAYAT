...
<head>
    ...
    body {
        ...
        background-image: url("http://localhost/Mini_Project/Images/F3.jpeg");
        width: 100%;
        /* Adjust as needed */
        height: 100%;
        /* Adjust as needed */
...
$sql = "SELECT COUNT(*) AS totalFamilies FROM family_details";
...
$sql = "SELECT FamilyMembers FROM family_details";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
$sql = "SELECT SUM(Voters) AS totalVoters FROM family_details";
$stmt = $conn->prepare($sql);
$stmt->execute();
$totalVoters = $stmt->fetch(PDO::FETCH_ASSOC)['totalVoters'];

// SQL query to get the sum of educated people for the entire dataset
$sql = "SELECT SUM(Education) AS totalLiterature FROM family_details";
$stmt = $conn->prepare($sql);
$stmt->execute();
