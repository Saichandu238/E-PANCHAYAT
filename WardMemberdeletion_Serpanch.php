...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $WardMember_id = $_POST['WardMember_id'];

    // Prepare and execute the delete query
    $sql = "DELETE FROM wardmemberdetails WHERE ID = ? and Village= '$Village'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $WardMember_id);

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $deletion_message = "wardmember details with Ward Member ID $WardMember_id have been successfully deleted.";
        } else {
            $deletion_message = "No WardMember  found with wardmember id $WardMember_id.";
        }
    } else {
...
<head>
    ...
    <title>Delete wardmember Details</title>
    <style>
    body {
...
<body>
    ...
    <div class="delete-sro-box">
        <h3>Delete WardMember Details</h3>
        <form method="POST" action="">
            <label for="WardMember_id">Enter WardMember ID:</label>
            <input type="text" id="WardMember_id" name="WardMember_id" required>
