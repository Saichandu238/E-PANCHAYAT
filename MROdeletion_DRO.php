...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "DELETE FROM mro_details WHERE Employee_Id = ? and District= '$District'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $employee_id);

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $deletion_message = "MRO details with Employee ID $employee_id have been successfully deleted.";
        } else {
            $deletion_message = "No MRO found with Employee ID $employee_id.";
        }
    } else {
...
<div class="delete-sro-box">
    <h3>Delete MRO Details</h3>
    <form method="POST" action="">
        <label for="employee_id">Enter MRO Employee ID:</label>
        <input type="text" id="employee_id" name="employee_id" required>
        <input type="submit" value="Delete MRO">
    </form>
