...
<head>
    ...
    <title>Update Serpanch Details</title>
    <style>
        body {
...
<div class="form-container">
    <h2>Update Serpanch Details</h2>
    <h4>Serpanch Id cannot be changed </h4>
    <?php
    session_start(); // Start the session

    // Check if the employeeId is set in the session
    if (!isset($_SESSION['Serpanch_id'])) {
    ...
    if ($mro) {
        ?>
        <form action="updateSerpanch_details2.php" method="POST">
            <label for="Serpanch_Id">Serpanch ID</label>
            <input type="text" name="Serpanch_Id" value="<?php echo htmlspecialchars($ID); ?>">
