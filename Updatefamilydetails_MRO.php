...
// $Aadhar_number=$_SESSION['Aadhar_number'];
            ...
            $sql = "SELECT * FROM family_details WHERE FCS = ?";
        ...
        <?php
                ...
                echo "<p>Family details not found for FSC number: " . htmlspecialchars($FCS) . ". <a href='Admin_Profile.php'>Click here to go back</a></p>";
                exit();
            }
        ...
        if (isset($_POST['update'])) {
            ...
            $sql = "UPDATE family_details SET headAadhar = ?, NameOfHead = ?, DOB = ?, Voters = ?, Education = ?, Village = ?, Mandal = ?, District = ?, familyMembers = ? WHERE FCS = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssssss", $headAadhar, $name, $DOB, $Voters, $Education, $Village, $Mandal, $District, $familyMembers, $FCS);

            if ($stmt->execute()) {

            $sql = "UPDATE authenticationdetails SET headAadhar = ?, NameOfHead = ?, DOB = ?, Voters = ?, Education = ?, Village = ?, Mandal = ?, District = ?, familyMembers = ? WHERE FCS = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssssss", $headAadhar, $name, $DOB, $Voters, $Education, $Village, $Mandal, $District, $familyMembers, $FCS);

            if ($stmt->execute()) {
                echo "Family Details Updated Successfully";
            } else {
